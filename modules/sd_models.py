...
def get_closet_checkpoint_match(search_string):
    ...
    found = sorted(
        [info for info in checkpoints_list.values() if search_string in info.title], key=lambda x: len(x.title)
    )
    if found:
        return found[0]
...
def load_model_weights(model, checkpoint_info: CheckpointInfo, state_dict, timer):
    ...
    devices.unet_needs_upcast = (
        shared.cmd_opts.upcast_sampling and devices.dtype == torch.float16 and devices.dtype_unet == torch.float16
    )
...
def repair_config(sd_config):
    ...
    if (
        getattr(sd_config.model.params.first_stage_config.params.ddconfig, "attn_type", None) == "vanilla-xformers"
