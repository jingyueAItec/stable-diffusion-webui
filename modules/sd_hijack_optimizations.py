...
from modules import sub_quadratic_attention
...
class SdOptimizationSdpNoMem(SdOptimization):
    ...
    def is_available(self):
        return hasattr(torch.nn.functional, "scaled_dot_product_attention") and callable(
...
def sub_quad_attention(
    q, k, v, q_chunk_size=1024, kv_chunk_size=None, kv_chunk_size_min=None, chunk_threshold=None, use_checkpoint=True
):
    ...
    if chunk_threshold is None:
        chunk_threshold_bytes = (
            int(get_available_vram() * 0.9) if q.device.type == "mps" else int(get_available_vram() * 0.7)
        )
