    ...
    image = Image.fromarray(
        cv2.cvtColor(
            exposure.match_histograms(
                cv2.cvtColor(np.asarray(original_image), cv2.COLOR_RGB2LAB), correction, channel_axis=2
            ),
            cv2.COLOR_LAB2RGB,
        ).astype("uint8")
    )
...
class StableDiffusionProcessing:
    ...
    def unclip_image_conditioning(self, source_image):
        ...
        if self.sd_model.noise_augmentor is not None:
            ...
            c_adm, noise_level_emb = self.sd_model.noise_augmentor(
                c_adm, noise_level=repeat(torch.tensor([noise_level]).to(c_adm.device), "1 -> b", b=c_adm.shape[0])
            )
    ...
    def setup_prompts(self):
        ...
        self.all_negative_prompts = [
            shared.prompt_styles.apply_negative_styles_to_prompt(x, self.styles) for x in self.all_negative_prompts
        ]
    ...
    def get_conds_with_caching(self, function, required_prompts, steps, cache):
        ...
        if (
            cache[0] is not None
            and (required_prompts, steps, opts.CLIP_stop_at_last_layers, shared.sd_model.sd_checkpoint_info) == cache[0]
        ):
            return cache[1]
...
class Processed:
    def __init__(
        ...
        self.subseed = (
            int(self.subseed if type(self.subseed) != list else self.subseed[0]) if self.subseed is not None else -1
        )
...
def create_infotext(p, all_prompts, all_seeds, all_subseeds, comments=None, iteration=0, position_in_batch=0):
    ...
    generation_params = {
        ...
        "Model hash": getattr(
            ...
            "sd_model_hash",
            None
            if not opts.add_model_hash_to_info or not shared.sd_model.sd_model_hash
            else shared.sd_model.sd_model_hash,
        ),
        "Model": (
            None
            if not opts.add_model_name_to_info or not shared.sd_model.sd_checkpoint_info.model_name
            else shared.sd_model.sd_checkpoint_info.model_name.replace(",", "").replace(":", "")
        ),
        ...
        "Conditional mask weight": getattr(p, "inpainting_mask_weight", shared.opts.inpainting_mask_weight)
    ...
    generation_params_text = ", ".join(
        [
            k if k == v else f"{k}: {generation_parameters_copypaste.quote(v)}"
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if (
                    hasattr(p, "mask_for_overlay")
                    and p.mask_for_overlay
                    and any([opts.save_mask, opts.save_mask_composite, opts.return_mask, opts.return_mask_composite])
                ):
                    image_mask = p.mask_for_overlay.convert("RGB")
                    image_mask_composite = Image.composite(
                        image.convert("RGBA").convert("RGBa"),
                        Image.new("RGBa", image.size),
                        images.resize_image(2, p.mask_for_overlay, image.width, image.height).convert("L"),
                    ).convert("RGBA")
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        ...
        latent_scale_mode = (
            ...
            if self.hr_upscaler is not None
            else shared.latent_upscale_modes.get(shared.latent_upscale_default_mode, "nearest")
        )
        if self.enable_hr and latent_scale_mode is None:
            assert (
                len([x for x in shared.sd_upscalers if x.name == self.hr_upscaler]) > 0
        ...
        samples = self.sampler.sample(
            self, x, conditioning, unconditional_conditioning, image_conditioning=self.txt2img_image_conditioning(x)
        )
    ...
    def setup_prompts(self):
        ...
        self.all_hr_negative_prompts = [
            shared.prompt_styles.apply_negative_styles_to_prompt(x, self.styles) for x in self.all_hr_negative_prompts
        ]
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.initial_noise_multiplier = (
            opts.initial_noise_multiplier if initial_noise_multiplier is None else initial_noise_multiplier
        )
        self.mask = None
        self.nmask = None
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        for img in self.init_images:
            ...
            if image_mask is not None:
                ...
                image_masked.paste(
                    image.convert("RGBA").convert("RGBa"), mask=ImageOps.invert(self.mask_for_overlay.convert("L"))
                )
        ...
        if self.resize_mode == 3:
            self.init_latent = torch.nn.functional.interpolate(
                self.init_latent, size=(self.height // opt_f, self.width // opt_f), mode="bilinear"
            )
        ...
        if image_mask is not None:
            ...
            if self.inpainting_fill == 2:
                self.init_latent = (
                    ...
                    + create_random_tensors(self.init_latent.shape[1:], all_seeds[0 : self.init_latent.shape[0]])
                    * self.nmask
                )
    ...
    def get_token_merging_ratio(self, for_hr=False):
        return (
            ...
            or ("token_merging_ratio" in self.override_settings and opts.token_merging_ratio)
