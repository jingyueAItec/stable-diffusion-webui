...
if has_mps:
    ...
    CondFunc(
        ...
        lambda _, size, dtype, device, seed: device.type == "mps",
    )
    ...
    if version.parse(torch.__version__) < version.parse("1.13"):
        ...
        CondFunc(
            "torch.Tensor.to",
            lambda orig_func, self, *args, **kwargs: orig_func(self.contiguous(), *args, **kwargs),
        ...
        CondFunc(
            ...
            lambda orig_func, *args, **kwargs: orig_func(*([args[0].contiguous()] + list(args[1:])), **kwargs),
            lambda _, *args, **kwargs: args and isinstance(args[0], torch.Tensor) and args[0].device.type == "mps",
        )
        # MPS workaround for https://github.com/pytorch/pytorch/issues/90532
        CondFunc(
            "torch.Tensor.numpy",
            lambda orig_func, self, *args, **kwargs: orig_func(self.detach(), *args, **kwargs),
        ...
        CondFunc(
            ...
            lambda _, input, *args, **kwargs: len(args) == 4 and input.device.type == "mps",
        )
            ...
            for funcName in ["torch.argmax", "torch.Tensor.argmax"]:
                CondFunc(
                    ...
                    lambda _, input, *args, **kwargs: torch.max(
                        input.float() if input.dtype == torch.int64 else input, *args, **kwargs
                    )[1],
