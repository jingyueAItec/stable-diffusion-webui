...
from modules.script_callbacks import cfg_after_cfg_callback
from modules.script_callbacks import cfg_denoised_callback
from modules.script_callbacks import cfg_denoiser_callback
from modules.script_callbacks import CFGDenoisedParams
...
samplers_k_diffusion = [
    ...
    (
        ...
        "sample_dpmpp_2m_sde",
        ["k_dpmpp_2m_sde_ka"],
        {"brownian_noise": True, "discard_next_to_last_sigma": True},
    ),
    ...
    (
        ...
        "sample_dpmpp_2m_sde",
        ["k_dpmpp_2m_sde_ka"],
        {"scheduler": "karras", "brownian_noise": True, "discard_next_to_last_sigma": True},
    ),
]

samplers_data_k_diffusion = [
    sd_samplers_common.SamplerData(
        label, lambda model, funcname=funcname: KDiffusionSampler(funcname, model), aliases, options
    )
...
class CFGDenoiser(torch.nn.Module):
    ...
    def forward(self, x, sigma, uncond, cond, cond_scale, s_min_uncond, image_cond):
        ...
        if not is_edit_model:
            ...
            image_cond_in = torch.cat(
                [torch.stack([image_cond[i] for _ in range(n)]) for i, n in enumerate(repeats)]
                + [image_uncond]
                + [torch.zeros_like(self.init_latent)]
            )

        denoiser_params = CFGDenoiserParams(
            x_in, image_cond_in, sigma_in, state.sampling_step, state.sampling_steps, tensor, uncond
        )
            ...
            if shared.batch_cond_uncond:
                ...
                for batch_offset in range(0, x_out.shape[0], batch_size):
                    ...
                    x_out[a:b] = self.inner_model(
                        x_in[a:b], sigma_in[a:b], cond=make_condition_dict([cond_in[a:b]], image_cond_in[a:b])
                    )
        else:
            x_out = torch.zeros_like(x_in)
    ...
    def get_sigmas(self, p, steps):
        ...
        if p.sampler_noise_scheduler_override:
            ...
            sigma_min, sigma_max = (
                ...
                else (self.model_wrap.sigmas[0].item(), self.model_wrap.sigmas[-1].item())
            )
