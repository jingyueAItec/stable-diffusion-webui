...
from modules.call_queue import wrap_gradio_gpu_call_with_request
...
def create_seed_inputs(target_interface):
    ...
    random_seed.click(
        fn=None,
        _js="function(){setRandomSeed('" + target_interface + "_seed')}",
        show_progress=False,
        inputs=[],
        outputs=[],
    )
    random_subseed.click(
        fn=None,
        _js="function(){setRandomSeed('" + target_interface + "_subseed')}",
        show_progress=False,
        inputs=[],
        outputs=[],
    )
...
def connect_reuse_seed(
    seed: gr.Number, reuse_seed: gr.Button, generation_info: gr.Textbox, dummy_component, is_subseed
):
...
def update_token_counter(text, steps):
    ...
    token_count, max_length = max(
        [model_hijack.get_prompt_lengths(prompt) for prompt in prompts], key=lambda args: args[0]
    )
...
def create_sampler_and_steps_selection(choices, tabname):
    if opts.samplers_in_dropdown:
        with FormRow(elem_id=f"sampler_selection_{tabname}"):
            ...
            steps = gr.Slider(
                minimum=1, maximum=150, step=1, elem_id=f"{tabname}_steps", label="Sampling steps", value=20
            )
    else:
...
def ordered_ui_categories():
    ...
    for _, category in sorted(
        enumerate(shared.ui_reorder_categories), key=lambda x: user_order.get(x[1], x[0] * 2 + 0)
    ):
        yield category
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        txt_prompt_img = gr.File(
            label="", elem_id="txt2img_prompt_image", file_count="single", type="binary", visible=False
        )
        ...
        with gr.Row().style(equal_height=False):
                ...
                for category in ordered_ui_categories():
                    if category == "sampler":
                        ...
                        with FormGroup(visible=False, elem_id="txt2img_hires_fix") as hr_options:
                            ...
                            with FormRow(
                                elem_id="txt2img_hires_fix_row3", variant="compact", visible=opts.hires_fix_show_sampler
                            ) as hr_sampler_container:
                                hr_sampler_index = gr.Dropdown(
                                    label="Hires sampling method",
                                    elem_id="hr_sampler",
                                    choices=["Use same sampler"] + [x.name for x in samplers_for_img2img],
                                    value="Use same sampler",
                                    type="index",
                                )

                            with FormRow(
                                elem_id="txt2img_hires_fix_row4", variant="compact", visible=opts.hires_fix_show_prompts
            ...
            txt2img_args = dict(
                fn=wrap_gradio_gpu_call_with_request(modules.txt2img.txt2img, extra_outputs=[None, "", ""]),
                _js="submit",
                inputs=[
                    dummy_component,
                    txt2img_prompt,
            ...
            txt_prompt_img.change(
                fn=modules.images.image_data, inputs=[txt_prompt_img], outputs=[txt2img_prompt, txt_prompt_img]
            )
            ...
            txt2img_paste_fields = [
                ...
                (
                    ...
                    lambda d: gr.update(visible=True)
                    if d.get("Hires prompt", "") != "" or d.get("Hires negative prompt", "") != ""
                    else gr.update(),
                ),
            ...
            parameters_copypaste.register_paste_params_button(
                parameters_copypaste.ParamBinding(
                    paste_button=txt2img_paste,
                    tabname="txt2img",
                    source_text_component=txt2img_prompt,
            ...
            token_button.click(
                fn=wrap_queued_call(update_token_counter), inputs=[txt2img_prompt, steps], outputs=[token_counter]
            )
        ...
        with FormRow().style(equal_height=False):
            with gr.Column(variant="compact", elem_id="img2img_settings"):
                ...
                with gr.Tabs(elem_id="mode_img2img"):
                    ...
                    with gr.TabItem(
                        "Inpaint sketch", id="inpaint_sketch", elem_id="img2img_inpaint_sketch_tab"
                        ...
                        inpaint_color_sketch.change(
                            update_orig, [inpaint_color_sketch, inpaint_color_sketch_orig], inpaint_color_sketch_orig
                        )

                    with gr.TabItem(
                        "Inpaint upload", id="inpaint_upload", elem_id="img2img_inpaint_upload_tab"
                ...
                with FormRow():
                    resize_mode = gr.Radio(
                        label="Resize mode",
                        elem_id="resize_mode",
                        choices=["Just resize", "Crop and resize", "Resize and fill", "Just resize (latent upscale)"],
                        type="index",
                        value="Just resize",
                    )
            ...
            img2img_prompt_img.change(
                fn=modules.images.image_data, inputs=[img2img_prompt_img], outputs=[img2img_prompt, img2img_prompt_img]
            )

            img2img_args = dict(
                fn=wrap_gradio_gpu_call_with_request(modules.img2img.img2img, extra_outputs=[None, "", ""]),
                _js="submit_img2img",
                inputs=[
                    dummy_component,
                    dummy_component,
            ...
            parameters_copypaste.register_paste_params_button(
                parameters_copypaste.ParamBinding(
                    ...
                    tabname="img2img",
                    source_text_component=img2img_prompt,
                    source_image_component=None,
                )
            )
    ...
    with gr.Blocks(analytics_enabled=False) as modelmerger_interface:
        with gr.Row().style(equal_height=False):
            with gr.Column(variant="compact"):
                interp_description = gr.HTML(
                    value=update_interp_description("Weighted sum"), elem_id="modelmerger_interp_description"
                )

                with FormRow(elem_id="modelmerger_models"):
                    primary_model_name = gr.Dropdown(
                    ...
                    tertiary_model_name = gr.Dropdown(
                        modules.sd_models.checkpoint_tiles(),
            ...
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Create hypernetwork", id="create_hypernetwork"):
                    ...
                    new_hypernetwork_activation_func = gr.Dropdown(
                        ...
                        choices=modules.hypernetworks.ui.keys,
                    ...
                    new_hypernetwork_initialization_option = gr.Dropdown(
                        ...
                        choices=["Normal", "KaimingUniform", "KaimingNormal", "XavierUniform", "XavierNormal"],
                ...
                with gr.Tab(label="Preprocess images", id="preprocess_images"):
                    ...
                    with gr.Row(visible=False) as process_split_extra_row:
                        process_split_threshold = gr.Slider(
                            ...
                            maximum=1.0,
                            step=0.05,
                            elem_id="train_process_split_threshold",
                        )
                        process_overlap_ratio = gr.Slider(
                            ...
                            maximum=0.9,
                            step=0.05,
                            elem_id="train_process_overlap_ratio",
                        )
                    ...
                    with gr.Row(visible=False) as process_focal_crop_row:
                        process_focal_crop_face_weight = gr.Slider(
                            ...
                            maximum=1.0,
                            step=0.05,
                            elem_id="train_process_focal_crop_face_weight",
                        )
                        process_focal_crop_entropy_weight = gr.Slider(
                            ...
                            maximum=1.0,
                            step=0.05,
                            elem_id="train_process_focal_crop_entropy_weight",
                        )
                        process_focal_crop_edges_weight = gr.Slider(
                            ...
                            maximum=1.0,
                            step=0.05,
                            elem_id="train_process_focal_crop_edges_weight",
                        )
                    ...
                    with gr.Column(visible=False) as process_multicrop_col:
                        ...
                        with gr.Row():
                            process_multicrop_mindim = gr.Slider(
                                ...
                                value=384,
                                elem_id="train_process_multicrop_mindim",
                            )
                            process_multicrop_maxdim = gr.Slider(
                                ...
                                value=768,
                                elem_id="train_process_multicrop_maxdim",
                            )
                        with gr.Row():
                            process_multicrop_minarea = gr.Slider(
                                ...
                                value=64 * 64,
                                elem_id="train_process_multicrop_minarea",
                            )
                            process_multicrop_maxarea = gr.Slider(
                                ...
                                value=640 * 640,
                                elem_id="train_process_multicrop_maxarea",
                            )
                        with gr.Row():
                            process_multicrop_objective = gr.Radio(
                                ["Maximize area", "Minimize error"],
                                value="Maximize area",
                            ...
                            process_multicrop_threshold = gr.Slider(
                                ...
                                value=0.1,
                                elem_id="train_process_multicrop_threshold",
                            )
                ...
                with gr.Tab(label="Train", id="train"):
                    ...
                    with FormRow():
                        clip_grad_mode = gr.Dropdown(
                            value="disabled", label="Gradient Clipping", choices=["disabled", "value", "norm"]
                        )
    ...
    def run_settings(*args):
        ...
        return (
            opts.dumpjson(),
            f'{len(changed)} settings changed{": " if len(changed) > 0 else ""}{", ".join(changed)}.',
        )
    ...
    with gr.Blocks(theme=shared.gradio_theme, analytics_enabled=False, title="GreatLeap.AI") as demo:
        ...
        def update_image_cfg_scale_visibility():
            return gr.update(visible=shared.sd_model and shared.sd_model.cond_stage_key == "edit")
        ...
        def modelmerger(*args):
            try:
                ...
                modules.sd_models.list_models()
                return [
                    *[gr.Dropdown.update(choices=modules.sd_models.checkpoint_tiles()) for _ in range(4)],
...
def setup_ui_api(app):
    ...
    app.add_api_route(
        "/internal/quicksettings-hint", quicksettings_hint, methods=["GET"], response_model=List[QuicksettingsHint]
    )
