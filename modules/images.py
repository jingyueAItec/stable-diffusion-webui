...
def draw_grid_annotations(im, width, height, hor_texts, ver_texts, margin=0):
    ...
    ver_text_heights = [
        sum([line.size[1] + line_spacing for line in lines]) - line_spacing * len(lines) for lines in ver_texts
    ]

    pad_top = 0 if sum(hor_text_heights) == 0 else max(hor_text_heights) + line_spacing * 2

    result = Image.new(
        "RGB", (im.width + pad_left + margin * (cols - 1), im.height + pad_top + margin * (rows - 1)), "white"
    )
...
def draw_prompt_matrix(im, width, height, all_prompts, margin=0):
    ...
    hor_texts = [
        [GridAnnotation(x, is_active=pos & (1 << i) != 0) for i, x in enumerate(prompts_horiz)]
        for pos in range(1 << len(prompts_horiz))
    ]
    ver_texts = [
        [GridAnnotation(x, is_active=pos & (1 << i) != 0) for i, x in enumerate(prompts_vert)]
...
class FilenameGenerator:
    replacements = {
        ...
        if self.p.n_iter == 1 and self.p.batch_size == 1
