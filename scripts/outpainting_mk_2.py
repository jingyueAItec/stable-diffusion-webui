...
def get_matched_noise(_np_src_image, np_mask_rgb, noise_q=1, color_variation=0.05):
    ...
    brightness_variation = (
        0.0  # color_variation # todo: temporarily tieing brightness variation to color variation for now
    )
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        pixels = gr.Slider(
            label="Pixels to expand", minimum=8, maximum=256, step=8, value=128, elem_id=self.elem_id("pixels")
        )
        mask_blur = gr.Slider(
            label="Mask blur", minimum=0, maximum=64, step=1, value=8, elem_id=self.elem_id("mask_blur")
        )
    ...
    def run(self, p, _, pixels, mask_blur, direction, noise_q, color_variation):
        ...
        def expand(init, count, expand_pixels, is_left=False, is_right=False, is_top=False, is_bottom=False):
            ...
            for n in range(count):
                ...
                output_images.append(Image.fromarray(np.clip(noised * 255.0, 0.0, 255.0).astype(np.uint8), mode="RGB"))
            ...
            for n in range(count):
                output_images[n].paste(
                    ...
                    (
                        ...
                        0 if is_top else output_images[n].height - proc.images[n].height,
                    ),
                )
        ...
        if opts.samples_save:
            for img in all_processed_images:
                images.save_image(
                    img, p.outpath_samples, "", res.seed, p.prompt, opts.samples_format, info=res.info, p=p
                )
