...
class ScriptPostprocessingUpscale(scripts_postprocessing.ScriptPostprocessing):
                ...
                with gr.Tabs(elem_id="extras_resize_mode"):
                    ...
                    with gr.TabItem("Scale to", elem_id="extras_scale_to_tab") as tab_scale_to:
                        with FormRow():
                            ...
                            with gr.Column(
                                elem_id="upscaling_dimensions_row", scale=1, elem_classes="dimensions-tools"
                            ):
    ...
    def upscale(
        self, image, info, upscaler, upscale_mode, upscale_by, upscale_to_width, upscale_to_height, upscale_crop
    ):
        if upscale_mode == 1:
        ...
        if upscale_mode == 1 and upscale_crop:
            ...
            cropped.paste(
                image, box=(upscale_to_width // 2 - image.width // 2, upscale_to_height // 2 - image.height // 2)
            )
            image = cropped
    ...
    def process(
        ...
        upscaled_image = self.upscale(
            pp.image, pp.info, upscaler1, upscale_mode, upscale_by, upscale_to_width, upscale_to_height, upscale_crop
        )
    ...
    def ui(self):
        with FormRow():
            upscaler_name = gr.Dropdown(
                label="Upscaler", choices=[x.name for x in shared.sd_upscalers], value=shared.sd_upscalers[0].name
            )
