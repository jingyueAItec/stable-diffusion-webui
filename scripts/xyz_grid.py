...
def find_vae(name: str):
    ...
    if name.lower() == "none":
        ...
        choices = [
            x for x in sorted(modules.sd_vae.vae_dict, key=lambda x: len(x)) if name.lower().strip() in x.lower()
        ]
        if len(choices) == 0:
...
def draw_xyz_grid(
    ...
    for i in range(z_count):
        ...
        if draw_legend:
            grid = images.draw_grid_annotations(
                grid,
                processed_result.images[start_index].size[0],
    ...
    if draw_legend:
        z_grid = images.draw_grid_annotations(
            z_grid, sub_grid_size[0], sub_grid_size[1], title_texts, [[images.GridAnnotation()]]
        )
...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
            ...
            with gr.Column(scale=19):
                with gr.Row():
                    ...
                    fill_x_button = ToolButton(
                        value=fill_values_symbol, elem_id="xyz_grid_fill_x_tool_button", visible=False
                    )
                ...
                with gr.Row():
                    ...
                    fill_y_button = ToolButton(
                        value=fill_values_symbol, elem_id="xyz_grid_fill_y_tool_button", visible=False
                    )
                ...
                with gr.Row():
                    ...
                    fill_z_button = ToolButton(
                        value=fill_values_symbol, elem_id="xyz_grid_fill_z_tool_button", visible=False
                    )
        ...
        with gr.Row(variant="compact", elem_id="axis_options"):
            with gr.Column():
                ...
                no_fixed_seeds = gr.Checkbox(
                    label="Keep -1 for seeds", value=False, elem_id=self.elem_id("no_fixed_seeds")
                )
            with gr.Column():
                include_lone_images = gr.Checkbox(
                    label="Include Sub Images", value=False, elem_id=self.elem_id("include_lone_images")
                )
                include_sub_grids = gr.Checkbox(
                    label="Include Sub Grids", value=False, elem_id=self.elem_id("include_sub_grids")
                )
            with gr.Column():
        ...
        def select_axis(axis_type, axis_values_dropdown):
            ...
            return (
                ...
                gr.Textbox.update(visible=not has_choices),
                gr.update(choices=choices if has_choices else None, visible=has_choices, value=current_values),
            )

        x_type.change(
            fn=select_axis, inputs=[x_type, x_values_dropdown], outputs=[fill_x_button, x_values, x_values_dropdown]
        )
        y_type.change(
            fn=select_axis, inputs=[y_type, y_values_dropdown], outputs=[fill_y_button, y_values, y_values_dropdown]
        )
        z_type.change(
            fn=select_axis, inputs=[z_type, z_values_dropdown], outputs=[fill_z_button, z_values, z_values_dropdown]
        )
    ...
    def run(
        ...
        Image.MAX_IMAGE_PIXELS = (
            None  # disable check in Pillow and rely on check below to allow large custom image sizes
        )
        ...
        def fix_axis_seeds(axis_opt, axis_list):
            if axis_opt.label in ["Seed", "Var. seed"]:
                return [
                    int(random.randrange(4294967294)) if val is None or val == "" or val == -1 else val
